	...
	public partial class Friends : IPhonePageExtender
	{
		...
		private void RefreshBindings()
		{
			...
			ItemsRequestedFriends.DataContext =
				App.IsolatedStorage.UserAccount.Snaps.Where(
					s =>
						s.MediaType == MediaType.FriendRequest && App.IsolatedStorage.UserAccount.Friends.All(f => f.Name != s.ScreenName))
					.GroupBy(x => x.ScreenName)
					.Select(y => y.First());
		...
		private void ButtonFriendDetails_Click(object sender, RoutedEventArgs e)
		{
			...
			Friend friend = (button.Tag is Friend)
				? button.Tag as Friend
				: null;
			if (friend == null)
				return;

			Navigation.NavigateTo(Navigation.NavigationTarget.FriendInfo,
				new Dictionary<string, string> {{"friend-name", friend.Name}});
		}
		...
		protected override async void OnNavigatedTo(NavigationEventArgs e)
		{
			...
			Dictionary<string, Best> bests = App.IsolatedStorage.FriendsBests;
